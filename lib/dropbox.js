...
if(Meteor.isServer){
  ...
  import accessToken from '/server/secret/keys'
  ...
  client = new Dropbox({
    accessToken: accessToken,  // Use your token here
    fetch: fetch
  });
}
...
Images = new FilesCollection({
  ...
  onBeforeUpload: function(fileRef){
    if (this.userId) {
      if (fileRef.size <= 250000 && /png|jpg|jpeg|webp/i.test(fileRef.extension)) {
        return true;
      }
      console.log('Please upload image, with size equal or less than 250KB')
      return 'Please upload image, with size equal or less than 250KB';
    } else {
