...
PetitionsSchema = new SimpleSchema({
    ...
    imageCover: {
      ...
      custom: function() {
        const v = this.value;
        if((v?.length || -1) > 0 && (v?.length || -1) <= 40) {
          return undefined;
        } else {
          throw new Meteor.Error('wrong-value', 'Cover picture required!')
        }
      }
    },
    video: {
      ...
      custom: function() {
        ...
        if(match || v.length===0) {
          ...
          throw new Meteor.Error('wrong-value', 'Please enter proper YouTube link or leave it blank')
        }
      }
    },
    milestone: {
      ...
      custom: function() {
        ...
        if(v > 0) {
          ...
          throw new Meteor.Error('wrong-value', 'Milestone must be above 0')
        }
      }
    },
    fields: {
